...
class BookingLog(PositionColumns, Base):
    ...
    cast-change produces a new row).

    - `booking_logs_performance_id_user_id_created_at_index`, a composite
      index covering this unboundedly-growing table's actual read pattern
      (filter by performance_id, sometimes narrowed further by user_id,
      ordered by created_at -- see booking_jobs.py's
      notify_upcoming_booking_status/_latest_unnotified_entries).
    - `fee >= 0`, mirroring the same constraint on `bookings.fee`
      (booking_logs.fee is always copied from an already-validated
      Booking.fee at write time).
    - `booking_type` is a native Postgres ENUM; `Mapped[str]` keeps every
      existing string comparison working.
    - `created_at`/`updated_at` are TIMESTAMPTZ: `created_at` is populated
      ...
      Python. `notified_at` is TIMESTAMPTZ too but stays Python-managed:
      it is set by the scheduled `notify_upcoming_booking_status` job once
      a user has been emailed about this transition (see booking_jobs.py).

    `performance_id`/`user_id` are nullable, ON DELETE SET NULL foreign
    keys: a deleted Performance or User does not remove or block deletion
    of its historical booking_log rows -- they survive as orphaned
    (NULL-referencing) audit entries, matching this table's append-only
    nature.

    The position is one of three mutually-exclusive nullable foreign keys
