...
class Performance(Base):
    """A scheduled performance, with these DB-level guarantees:
    ...
    - Four money CheckConstraints (`choirjob_defaultfee >= 0`,
      ...
      the constraint must keep allowing NULL), each also enforced at the
      Pydantic layer (PerformanceRequest's `Field(ge=0)`s).
    ...
    `ordinariumwork_id`/`artist_id` are ON DELETE RESTRICT foreign keys:
    coreelement_service.
    ...
    (chosen per-booking on the Cast page) -- they're
